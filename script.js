...
<p>By achieving these landmark features, OBELiSC aims to make secure computation practical for deployment over large-scale networks, ultimately realizing a world where <strong>privacy-by-default</strong> becomes the standard for distributed computations.</p>`;
  ...
  setupEventListeners() {
    ...
    // Previous publications filter buttons - only handle main filter buttons, not sub-filter buttons
    document
      .querySelectorAll(".filter-btn-prev:not(.sub-filter-btn)")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          document
            .querySelectorAll(".filter-btn-prev:not(.sub-filter-btn)")
...
${processedAfter}`;
  ...
  filterPreviousPublications(filter) {
    ...
    if (!isSubFilter) {
      // Main filter selected - update main filter buttons and show/hide sub-filters
      document
        .querySelectorAll(".filter-btn-prev:not(.sub-filter-btn)")
        .forEach((b) => b.classList.remove("active"));
      const filterBtn = document.querySelector(
        `[data-filter="${filter}"].filter-btn-prev:not(.sub-filter-btn)`,
      );
      if (filterBtn) {
        filterBtn.classList.add("active");
      }
      this.updatePreviousWorkPackageFilters(filter);
    } else {
      // Sub-filter selected - just update sub-filter buttons
      document
        .querySelectorAll(".filter-btn-prev.sub-filter-btn")
        .forEach((b) => b.classList.remove("active"));
      const subFilterBtn = document.querySelector(
        `[data-filter="${filter}"].filter-btn-prev.sub-filter-btn`,
      );
      if (subFilterBtn) {
        subFilterBtn.classList.add("active");
      }
    }
  }

  updatePreviousWorkPackageFilters(filter) {
    let subFilters = document.querySelector(".previous-wp-sub-filters");
    ...
    if (filter === "rg1" || filter === "rg2" || filter === "rg3") {
      const filtersContainer = document.querySelector(
        ".previous-publications-filters",
      );

      // Create sub-filters container
      subFilters = document.createElement("div");
      subFilters.className = "previous-wp-sub-filters";

      let subFilterButtons = "";

      if (filter === "rg1") {
        subFilterButtons = `
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg1-specific">RG1: Specific</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg1-ole">RG1: OLE</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg1-unbounded">RG1: Unbounded</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg1-multiparty">RG1: Multiparty</button>
        `;
      } else if (filter === "rg2") {
        subFilterButtons = `
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg2-nisp">RG2: NISP</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg2-nizk">RG2: NIZK</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg2-foundations">RG2: Foundations</button>
        `;
      } else if (filter === "rg3") {
        subFilterButtons = `
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg3-multiparty">RG3: Multiparty</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg3-general">RG3: General</button>
          <button class="filter-btn-prev sub-filter-btn" data-filter="rg3-barriers">RG3: Barriers</button>
        `;
      }
